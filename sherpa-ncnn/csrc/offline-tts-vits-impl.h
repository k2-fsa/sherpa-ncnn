 ...
 public:
  explicit OfflineTtsVitsImpl(const OfflineTtsConfig &config)
...
#if __OHOS__
          SHERPA_NCNN_LOGE("rule fst: %{public}s", f.c_str());
#else
          SHERPA_NCNN_LOGE("rule fst: %s", f.c_str());
#endif
        }
    ...
    if (!config.rule_fars.empty()) {
      if (config.model.debug) {
        SHERPA_NCNN_LOGE("Loading FST archives");
      }
      std::vector<std::string> files;
      ...
      for (const auto &f : files) {
        if (config.model.debug) {
#if __OHOS__
          SHERPA_NCNN_LOGE("rule far: %{public}s", f.c_str());
#else
          SHERPA_NCNN_LOGE("rule far: %s", f.c_str());
#endif
        }
      ...
      if (config.model.debug) {
        SHERPA_NCNN_LOGE("FST archives loaded!");
      }
    }
#endif
  }
  ...
  GeneratedAudio Generate(const TtsArgs &args,
...
#if __OHOS__
      SHERPA_NCNN_LOGE("Raw text: %{public}s", text.c_str());
#else
      SHERPA_NCNN_LOGE("Raw text: %s", text.c_str());
#endif
    }
    ...
    if (!tn_list_.empty()) {
      for (const auto &tn : tn_list_) {
        ...
        if (config_.model.debug) {
#if __OHOS__
          SHERPA_NCNN_LOGE("After normalizing: %{public}s", text.c_str());
#else
          SHERPA_NCNN_LOGE("After normalizing: %s", text.c_str());
#endif
        }
      }
    }
    ...
    if (token_ids.empty() ||
      ...
      SHERPA_NCNN_LOGE("Failed to convert %s to token IDs", text.c_str());
      return {};
    }
