// sherpa-ncnn/jni/wave-reader.cc
//
...
static jobjectArray ReadWaveImpl(JNIEnv *env, std::istream &is,
  ...
  std::vector<float> samples =
      sherpa_ncnn::ReadWave(is, &sampling_rate, &is_ok);

  if (!is_ok) {
    SHERPA_NCNN_LOGE("Failed to read '%s'", p_filename);
...
Java_com_k2fsa_sherpa_ncnn_WaveReader_00024Companion_readWaveFromFile(
...
SHERPA_NCNN_EXTERN_C
JNIEXPORT jobjectArray JNICALL
Java_com_k2fsa_sherpa_ncnn_WaveReader_readWaveFromFile(JNIEnv *env,
                                                       jclass /*obj*/,
                                                       jstring filename) {
  return Java_com_k2fsa_sherpa_ncnn_WaveReader_00024Companion_readWaveFromFile(
      env, nullptr, filename);
}

SHERPA_NCNN_EXTERN_C
JNIEXPORT jobjectArray JNICALL
Java_com_k2fsa_sherpa_ncnn_WaveReader_00024Companion_readWaveFromAsset(
  ...
  std::vector<char> buffer = sherpa_ncnn::ReadFile(mgr, p_filename);
